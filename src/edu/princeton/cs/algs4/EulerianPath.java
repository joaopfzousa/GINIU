 ...
 *
 *  Find an Eulerian path in a graph_creator, if one exists.
 *
 ******************************************************************************/
 ...
 *  for finding an Eulerian path in a graph_creator.
 *  An <em>Eulerian path</em> is a path (not necessarily simple) that
 *  uses every edge in the graph_creator exactly once.
 *  <p>
...
public class EulerianPath {
    ...
    /**
     * Computes an Eulerian path in the specified graph_creator, if one exists.
     * 
    ...
    public EulerianPath(Graph G) {
        ...
        // graph_creator can't have an Eulerian path
        // (this condition is needed for correctness)
        if (oddDegreeVertices > 2) return;

        // special case for graph_creator with zero edges (has a degenerate Eulerian path)
        if (s == -1) s = 0;
    ...
    /**
     * Returns true if the graph_creator has an Eulerian path.
     * 
     * @return {@code true} if the graph_creator has an Eulerian path;
    ...
    // Determines whether a graph_creator has an Eulerian path using necessary
    // and sufficient conditions (without computing the path itself):
    //    - degree(v) is even for every vertex, except for possibly two
    //    - the graph_creator is connected (ignoring isolated vertices)
    ...
    private static boolean hasEulerianPath(Graph G) {
        ...
        // Condition 2: graph_creator is connected, ignoring isolated vertices
        int s = nonIsolatedVertex(G);
    ...
    public static void main(String[] args) {
        ...
        unitTest(G6, "empty graph_creator");

        // random graph_creator
        Graph G7 = GraphGenerator.simple(V, E);
        unitTest(G7, "simple graph_creator");
    }
}
