 ...
 *  in a bipartite graph_creator using Hopcroft-Karp algorithm.
 *
 ******************************************************************************/
 ...
 *  <em>minimum (cardinality) vertex cover</em> in a bipartite graph_creator.
 *  A <em>bipartite graph_creator</em> in a graph_creator whose vertices can be partitioned
 *  into two disjoint sets such that every edge has one endpoint in either set.
 *  A <em>matching</em> in a graph_creator is a subset of its edges with no common
 *  vertices. A <em>maximum matching</em> is a matching with the maximum number
 *  of edges.
 *  A <em>perfect matching</em> is a matching which matches all vertices in the graph_creator.
 *  A <em>vertex cover</em> in a graph_creator is a subset of its vertices such that
 ...
 *  graph_creator, the maximum number of edges in matching equals the minimum number
 ...
 *  of vertices in the graph_creator. It uses extra space (not including the graph_creator)
...
public class HopcroftKarp {
    ...
    private final int V;                 // number of vertices in the graph_creator
     ...
     * in a bipartite graph_creator.
     *
     * @param  G the bipartite graph_creator
     * @throws IllegalArgumentException if {@code G} is not bipartite
     */
    public HopcroftKarp(Graph G) {
        bipartition = new BipartiteX(G);
        if (!bipartition.isBipartite()) {
            throw new IllegalArgumentException("graph_creator is not bipartite");
        }
        ...
        // the call to hasAugmentingPath() provides enough info to reconstruct level graph_creator
        while (hasAugmentingPath(G)) {
            ...
            for (int s = 0; s < V; s++) {
                ...
                while (!path.isEmpty()) {
                    ...
                    // retreat, no more edges in level graph_creator leaving v
                    if (!adj[v].hasNext())
                        path.pop();

                    // advance
                    else {
                        // process edge v-w only if it is an edge in level graph_creator
                        int w = adj[v].next();
   ...
   // is the edge v-w in the level graph_creator?
   ...
   // is the edge v-w a forward edge not in the matching or a reverse edge in the matching?
     ...
     *   - if so, upon termination adj[] contains the level graph_creator;
    ...
    /**
     * Returns true if the graph_creator contains a perfect matching.
     * That is, the number of edges in a maximum matching is equal to one half
     * of the number of vertices in the graph_creator (so that every vertex is matched).
     *
     * @return {@code true} if the graph_creator contains a perfect matching;
     *         {@code false} otherwise
     */
    ...
    private boolean certifySolution(Graph G) {
        ...
        // check that mate() uses only edges that appear in the graph_creator
        for (int v = 0; v < V; v++) {
            if (mate(v) == -1) continue;
     ...
     * creates a random bipartite graph_creator with {@code V1} + {@code V2} vertices
